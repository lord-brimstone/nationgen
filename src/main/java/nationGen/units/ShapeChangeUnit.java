...
public class ShapeChangeUnit extends Unit {
	...
	public void write(PrintWriter tw, String spritedir)
	{
		...
		for(Command c : thisForm.commands)
		{
			...
			if(c.command.equals("#spr1"))
			{
				if(c.args.get(0).equals("greyscale"))
				{
					...
					String outputFilepath = "./mods/" + spritedir + "/shapechange_" + id + "_a.tga";
					try {
						BufferedImage image = otherForm.render();
						Drawing.writeTGA(Drawing.greyscale(image, greyscaleunits), outputFilepath);
					} catch (IOException e) {
						throw new IllegalStateException("Couldn't write greyscaled image to '" + outputFilepath + "'");
					}
		...
		{
			String outputFilepath = "./mods/" + spritedir + "/shapechange_" + id + "_b.tga";
			try {
